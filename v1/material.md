...
### Получение остатка группы материалов (группа определяется определённым атрибутом, например код SKU)

Для получения материала необходимо отправить **`GET`** запрос на **`/material_sum/:query`**<br>
...
[
    {
        "_id": 1,
        "name": "Амоксициллин таб. 500 мг №20 (короб 126 уп.) Киргизия",
        "totalAmount": {
            "$numberDecimal": "206892.000"
        }
    }
]
```
...
Параметры:<br>
**query*** - запрос к базе, например: { "value.Код SKU": "01.JAT0003B02KG.001" }<br>
