...
Для получения списка групп справочников необходимо отправить **`GET`** запрос на **`/api/deskbook_group`**<br>

Пример ответа:<br>
```
[
    {
        "_id": "5d41368692d16b40f47dbcdb",
        "name": "Производство"
    },
    {
        "_id": "5d4136c192d16b40f47dbcdc",
        "name": "Заказы"
    }
]
```
...
### Создание группы для справочников

Для создания группы для справочников необходимо отправить **`POST`** запрос на **`/api/deskbook_group`**<br>
...
### Изменение названия группы для справочников

Для изменения названия группы для справочников необходимо отправить **`PUT`** запрос на **`/api/deskbook_group`**<br>

Пример отправки параметров:<br>
```
{
    "_id": "5d413f529238491a4026502b",
    "name": "Тест"
}
```
...
### Удаление группы для справочников

Для удаления группы для справочников необходимо отправить **`DELETE`** запрос на **`/api/deskbook_group/:id`**<br>
...
Для получения списка справочников необходимо отправить **`GET`** запрос на **`/api/deskbook/:g_id/:value`**<br>
...
**value*** - Имя справочника для поиска, если поиск не используется отправляем 'null'<br>
